...
func templates(api *confluence.API) (*template.Template, error) {
	...
	for name, body := range map[string]string{
		...
		`ac:isms:pageproperties`: text(
			...
			`<ac:parameter ac:name="id">{{ or .Id "report" }}</ac:parameter>`,
			`<ac:rich-text-body>`,
			`<table>`,
			`<tbody>`,
		...
		`ac:isms:pagedetails`: text(
			`<ac:structured-macro ac:name="details">`,
			`<ac:rich-text-body>`,
			`<table>`,
			`<tbody>`,
			`<tr>`,
			`{{ if .DocId }}<th>Document Number</th>{{ end }}`,
			`<th>Owner</th>`,
			`<th>Next Audit</th>`,
			`<th>Version</th>`,
			`{{ if eq .Status "GOOD" }}<th>Status</th>{{ end }}`,
			`{{ if eq .Status "SCHEDULED" }}<th>Status</th>{{ end }}`,
			`{{ if eq .Status "PENDING" }}<th>Status</th>{{ end }}`,
			`{{ if eq .Status "RISK" }}<th>Status</th>{{ end }}`,
			`<th>Restricted</th>`,
			`</tr>`,
			`<tr>`,
			`{{ if .DocId }}<td>{{ .DocId }}</td>{{ end }}`,
			`<td><ac:structured-macro ac:name="qc-property-macro-dynamic"><ac:parameter ac:name="property">Created by</ac:parameter></ac:structured-macro></td>`,
			`<td><ac:structured-macro ac:name="qc-expiration-date-macro"><ac:parameter ac:name="qc-expiration-message">OVERDUE</ac:parameter><ac:parameter ac:name="qc-expiry-warning-days">30</ac:parameter><ac:parameter ac:name="qc-expiration-date">{{ .ExpirationDate }}</ac:parameter></ac:structured-macro></td>`,
			`<td><ac:structured-macro ac:name="qc-property-macro-dynamic"><ac:parameter ac:name="property">Version</ac:parameter></ac:structured-macro></td>`,
			`{{ if eq .Status "GOOD" }}<td><ac:structured-macro ac:name="status"><ac:parameter ac:name="title">GOOD</ac:parameter><ac:parameter ac:name="colour">Green</ac:parameter></ac:structured-macro></td>{{ end }}`,
			`{{ if eq .Status "SCHEDULED" }}<td><ac:structured-macro ac:name="status"><ac:parameter ac:name="title">SCHEDULED</ac:parameter><ac:parameter ac:name="colour">Yellow</ac:parameter></ac:structured-macro></td>{{ end }}`,
			`{{ if eq .Status "PENDING" }}<td><ac:structured-macro ac:name="status"><ac:parameter ac:name="title">PENDING</ac:parameter><ac:parameter ac:name="colour">Blue</ac:parameter></ac:structured-macro></td>{{ end }}`,
			`{{ if eq .Status "RISK" }}<td><ac:structured-macro ac:name="status"><ac:parameter ac:name="title">RISK</ac:parameter><ac:parameter ac:name="colour">RED</ac:parameter></ac:structured-macro></td><{{ end }}`,
			`<td>{{ or .Restricted "NO" }}</td>`,
			`</tr>`,
			`</tbody>`,
			`</table>`,
			`</ac:rich-text-body>`,
